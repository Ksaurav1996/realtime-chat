  ...
  return (
    <>
      <button onClick={() => setOpen(true)}>
        <img className="w-[40px] h-[40px] rounded-full" src={getChatPhoto(activeChat, activeUser)} alt="Profile Pic" />
      </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded-lg w-[90%] max-w-md shadow-xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-3">
              <h2 className="text-lg font-semibold">{getChatName(activeChat, activeUser)}</h2>
              <RxCross2 className="cursor-pointer" onClick={() => setOpen(false)} />
            </div>
                ...
                <div className="flex flex-wrap gap-2 my-2">
                  {members.map((e) => (
                    <div key={e._id} className="flex items-center gap-1 bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded border border-green-400">
                      ...
                      <RxCross2 onClick={() => deleteSelected(e)} className="cursor-pointer" />
                    </div>
                  ))}
                </div>

                <form className="flex flex-col gap-3">
                  <input value={name} onChange={(e) => setName(e.target.value)} className="border p-2 text-sm rounded" placeholder="Group Name" required />
                  <input value={search} onChange={(e) => setSearch(e.target.value)} className="border p-2 text-sm rounded" placeholder="Add users" />
                </form>

                <Search isLoading={isLoading} handleClick={handleClick} search={search} searchResults={searchResults} />

                <div className="flex justify-end gap-2 mt-3">
                  <button onClick={updateBtn} className="bg-blue-600 text-white text-xs px-4 py-1 rounded">Update</button>
                  <button onClick={leaveGroup} className="bg-red-600 text-white text-xs px-4 py-1 rounded">Leave</button>
                </div>
              </>
            ) : (
              <div className="flex flex-col items-center">
                <img className="w-[70px] h-[70px] rounded-full shadow" src={getChatPhoto(activeChat, activeUser)} alt="" />
                <h2 className="text-lg font-semibold">{getChatName(activeChat, activeUser)}</h2>
                <h3 className="text-sm text-green-600">{!activeChat.isGroup && (activeChat.users[0]._id === activeUser.id ? activeChat.users[1].email : activeChat.users[0].email)}</h3>
                <p className="text-xs mt-2">{!activeChat.isGroup && (activeChat.users[0]._id === activeUser.id ? activeChat.users[1].bio : activeChat.users[0].bio)}</p>
              </div>
            )}
          </div>
        </div>
